...
export default function DashboardPage() {
    ...
    const { activities: recentActivities, loading: activitiesLoading } = useActivities(user?.id)
    const { milestones: upcomingMilestones, loading: milestonesLoading } = useMilestones(user?.id)
    ...
    const loading = projectsLoading || activitiesLoading || milestonesLoading || analyticsLoading
        ...
        <div className="space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between">
                    ...
                    <div className="flex gap-4 mt-4 md:mt-0">
                        ...
                        <button
                            ...
                            className="px-4 py-2 rounded-lg border border-white/10 hover:border-neon-green hover:bg-neon-green/10 transition-colors font-medium"
                        >
                            Quick Tour
                        </button>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-8">
                    ...
                    {/* Upcoming Milestones */}
                    <div className="glass-effect rounded-2xl p-6 border border-white/10" data-tour-id="upcoming-milestones">
                        <div className="flex items-center justify-between mb-6">
                                    ...
                                    <div key={milestone.id} className="p-4 rounded-lg bg-white/5">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className={`px-2 py-1 rounded-full text-xs ${milestone.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' :
                                                ...
                                                {milestone.status === 'pending' ? 'Pending' : 'Complete'}
                                            </span>
                                        </div>
                                        <p className="text-sm text-gray-400 mb-3">{milestone.projects?.name || 'Unknown Project'}</p>
                                        <div className="flex items-center justify-between text-sm">
                                            <span className="text-gray-400">
                                                Due: {milestone.due_date ? new Date(milestone.due_date).toLocaleDateString() : 'No date'}
                                            </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="glass-effect rounded-2xl p-6 border border-white/10" data-tour-id="quick-actions">
                ...
                <div className="glass-effect rounded-2xl p-6 border border-white/10" data-tour-id="performance-section">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="p-3 rounded-lg bg-gradient-to-r from-purple-500/20 to-pink-500/20">
            ...
            {/* Quick Tour Modal */}
            {showTour && (
                <QuickTourGuide onClose={() => setShowTour(false)} />
            )}
        </div>
    )
}
